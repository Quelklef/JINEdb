...
const migrations = [
  {
    ...
    alterations: [
      ...
      // Create a store for recipes called $recipes
      addStore<Recipe>('$recipes'),
      
      // Track recipes by their name
      // Require names to be unique
      addIndex<Recipe, string>('$recipes.$name', '.name', { unique: true });
      
      // Track recipes by their serving count
      addIndex<Recipe, number>('$recipes.$servings', '.servings');
...
interface $$ {
  $recipes: Store<Recipe> & {
    ...
    // Note this is Index<Recipe, string>, not Index<Recipe, Array<string>>
...
// Create our database
const jine = newJine<$$>('recipes', migrations);
...
await jcon.$recipes.$name.get('Basic Biscuits');  // returns `biscuits`
// .get only works on unique indexes and returns one item, or errors if no item is found

// I have some eggs I want to get rid of
await jcon.$recipes.$ingredients.find('eggs');  // [pancakes, waffles]
// .find returns all items matching a given trait

// I'm gonna have a lot of guests over
await jcon.$recipes.$servings.range({ above: 7 }).array();  // [biscuits, tacros]
// It's just me for this meal, and I don't want leftovers 
await jcon.$recipes.$servings.range({ below: 3 }).array();  // [] :(
// I want to try something complicated
await jcon.$recipes.$ingredient_count.range({ above: 10 }).array();  // [waffles]
